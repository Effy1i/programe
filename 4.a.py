...
class Constants:
    ...
    # --- 系数与阈值 ---
    ALPHA_I = 0.05    # 消防员血量衰减系数 (α_I)
    ALPHA_VD = 0.01    # 被困人员血量衰减系数 (α_VD)
    ALPHA_VS = 0.05    # 烟雾对速度影响系数 (α_VS)
    ALPHA_P = 0.005   # 专业性随血量损失的衰减系数 (α_P)
...
class SimulationCore:
    ...
    def run_simulation(self, chromosome):
        ...
        final_routes = {}  # 💥 修复 1: 定义 final_routes
        ...
        while active_firefighters:
            ...
            for F_k in active_firefighters[:]:  # 遍历副本以安全修改列表
                ...
                # --- A. 尝试执行 dt 时间步长的移动或搜救 ---
                # **重要：这需要一个复杂的函数来处理移动/搜救/状态切换**
            ...
            # 2. 被困人员并行状态更新 (基于 T_Total_Start + dt)
            for victim in self.victims:
